...
<body>
	<table>
		<tr><th>教員検索：</th>
		<td>
		<form action="/Hammerpoint/KyoukaKanri?action=kensaku2" method="post">
	...
	<c:choose>
			<c:when test="${check == 1 }">
				<tr><th>教員：</th>
				<td>
					<form action="/Hammerpoint/KyoukaKanri?action=${kyouka.kyoukaId }" method="post">
							<select name="kyouinSelect" style="width:200px; font-size:18px;">
								<option value="99999">教員を選択してください</option>
										...
										<c:choose>
												<c:when test="${kyouka.kyouinId != kyouin.kyouinId}">
					...
					<tr><th>クラス:</th>
									...
									<option value="999">クラスを選択してください</option>
										<c:choose>
											<c:when test="${check == 1 }">
												...
												<c:choose>
													<c:when test="${kyouka.classId != room.classId }">
						...
						<tr><th>教科名：</th>
							<td>
							<input type="text" name="kyoukaName" value="${kyouka.kyoukaName }" >
